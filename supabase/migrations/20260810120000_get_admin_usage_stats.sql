-- Server-side aggregation for the admin Usage Stats tab.
-- Previously the panel downloaded every user_swipes row (~63k and growing) to the
-- browser and aggregated in JS, which got slow as data grew. This RPC does the whole
-- aggregation in Postgres and returns ~one row per user plus the chart series, so the
-- payload is tiny and constant-ish regardless of swipe volume.
--
-- Guarded by is_current_user_admin() (same check the RLS "Admins can view all" policies
-- use); SECURITY DEFINER so it can read auth.users for last_sign_in_at.

create index if not exists idx_user_swipes_created_at on public.user_swipes (created_at);
